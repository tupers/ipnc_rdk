...
void GetCodecCombo(char *cCombo)
{
	...
	{
		if(pSysInfo->lan_config.nVideocombo==0)
			strcpy(cCombo, "H264 + MJPEG" );
		else if(pSysInfo->lan_config.nVideocombo==1)
			strcpy(cCombo, "MPEG4 + MJPEG" );
		else if(pSysInfo->lan_config.nVideocombo==2)
			strcpy(cCombo, "H264 + H264" );
		else if(pSysInfo->lan_config.nVideocombo==3)
			strcpy(cCombo, "MPEG4 + MPEG4" );
		else if(pSysInfo->lan_config.nVideocombo==3)
	...
	{
		if(pSysInfo->lan_config.nVideocombo==0)
			strcpy(cCombo, "H264 + H264 + MJPEG" );
		else if(pSysInfo->lan_config.nVideocombo==1)
			strcpy(cCombo, "MPEG4 + MPEG4 + MJPEG" );
		else
			strcpy(cCombo, "NULL");
	}
...
void GetResolution(char *resl)
{
       ...
       switch(pSysInfo->lan_config.nVideocodecmode)
       {
	     ...
	     case 3:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
		   	     strcpy(resl, "H264:1080p,JPEG:D1" );
		    else if(pSysInfo->lan_config.nVideocodecres == 1)
	     ...
	     case 4:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
		   	     strcpy(resl, "MPEG4:1080p,JPEG:D1" );
		    else if(pSysInfo->lan_config.nVideocodecres == 1)
	     ...
	     case 5:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
		   	     strcpy(resl, "H264:1080p,H264:D1" );
		    else if(pSysInfo->lan_config.nVideocodecres == 1)
	     ...
	     case 6:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
		   	     strcpy(resl, "MPEG4:1080p,MPEG4:D1" );
		 	else if(pSysInfo->lan_config.nVideocodecres == 1)
	     ...
	     case 7:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
		   		strcpy(resl, "H264:1080p,MPEG4:D1" );
			else
				strcpy(resl, "NULL");
	     ...
	     case 8:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
				strcpy(resl, "H264:1080P+D1, MJPEG:1080P");
		   	else if(pSysInfo->lan_config.nVideocodecres == 1)
				strcpy(resl, "H264:5MP+D1, MJPEG:5MP");
		   	else if(pSysInfo->lan_config.nVideocodecres == 2)
				strcpy(resl, "H264:8MP+D1, MJPEG:8MP");
		   	else if(pSysInfo->lan_config.nVideocodecres == 3)
				strcpy(resl, "H264:10MP+D1, MJPEG:10MP");
			else
				strcpy(resl, "NULL");
	     ...
	     case 9:
		   	if(pSysInfo->lan_config.nVideocodecres == 0)
				strcpy(resl, "MPEG4:1080P+D1, MJPEG:1080P");
			else
				strcpy(resl, "NULL");
...
void GetDynRange(char *dynRange)
{
	...
	{
		strcpy(dynRange, "High Quality");
	}
	else if(pSysInfo->advanceMode.dynRange == 1)
	{
		strcpy(dynRange, "High Speed");
	}
	else
	{
...
void GetWhiteBalance(char *wbalance)
{
     ...
     {
          strcpy(wbalance, "Day_D65");
     }
	 else if(pSysInfo->lan_config.nWhiteBalance== 2)
     {
          strcpy(wbalance, "Day_D55");
     }
	 else if(pSysInfo->lan_config.nWhiteBalance== 3)
     {
          strcpy(wbalance, "Florescent");
     }
	 else if(pSysInfo->lan_config.nWhiteBalance== 4)
     {
          strcpy(wbalance, "Incandescent");
     }
     else
     {
          strcpy(wbalance, "Auto");
...
void GetSNF(char *snf)
{
     ...
     {
		 if((pSysInfo->advanceMode.vnfMode!=0)||
			(pSysInfo->advanceMode.vnfMode!=3))
		 {
			   strcpy(snf, "ON");
		 }
		 else
...
void GetTnf(char *tnflt)
{
     ...
     if(pSysInfo->advanceMode.vnfEnable)
     {
		 if((pSysInfo->advanceMode.vnfMode!=1)||
			(pSysInfo->advanceMode.vnfMode!=4))
		 {
			   strcpy(tnflt, "ON");
		 }
		 else
...
int GetDMVAAlgo(char *dmvaalgo)
{
     SysInfo* pSysInfo = GetSysInfo();

	 if(pSysInfo->dmva_config.dmvaenable_sys == 0)
	 {
		 return -1;
	 }
...
void PutFrameRate(void *pOutMem, int MaxSize)
{
    ...
    if(pSysInfo->lan_config.nVideocodecmode >= 0 && pSysInfo->lan_config.nVideocodecmode <=2)
   	{
        sprintf(String, "%d fps", pSysInfo->lan_config.frameRateVal1);
	...
	else  if(pSysInfo->lan_config.nVideocodecmode >= 3 && pSysInfo->lan_config.nVideocodecmode <=7)
	{
        ...
        sprintf(String, "%d fps", pSysInfo->lan_config.frameRateVal2);
	...
	else
	{
        ...
        sprintf(String, "%d fps", pSysInfo->lan_config.frameRateVal3);
		sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Frame Rate(Stream3)", String, "5 fps");
		if( MaxSize > strlen(pOutMem) )
			strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
	}
}
...
int Curr_Param_List_To_html(void *pOutMem, int MaxSize)
{
	...
	GetStreamType(String);
      sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Stream Type", String, "Tri-Stream");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));

	GetCodecCombo(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Codec Combo", String, "H264 + H264 + MJPEG");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));

	GetResolution(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Resolution", String, "H264:1080P+D1, MJPEG:1080P");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
	...
	GetTnf(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Temporal Noise Filter", String, "OFF");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
