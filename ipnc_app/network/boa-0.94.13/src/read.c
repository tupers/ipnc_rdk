...
int read_header(request * req, node_t * node)
{
...
#ifdef VERY_FASCIST_LOGGING
        log_error_time();
        fprintf(stderr, "status, check: %d, %c\n", req->status, *check);
#endif
    ...
    if (req->status < BODY_READ) {
        ...
        if(req->ssl == NULL){
			...
			/* [onvif change]it is not designed well, because the request we supposed coming from webGUI
			 * usually did not include Body,  the process of body is ignored, application may see the return
			 * value of read_header and do the free request, when soap message coming too fast, body
			 * will not be handle properly, here we added a workaround to make read_header receive complete
			 * data so that the onvif message parse will not meet problem */
			int iFirstBytes = bytes;
			while(bytes > 0)
			{
				usleep(2000);
				bytes = read(req->fd, buffer + req->client_stream_pos + bytes , buf_bytes_left - bytes);
				if(bytes > 0)
					req->client_stream_pos += bytes;
			}
			/* it is expected bytes > 0 which indicates sucess  */
			bytes = iFirstBytes;
