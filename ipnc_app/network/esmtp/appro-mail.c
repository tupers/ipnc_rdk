...
int CreateTestFile(char *strTestFileName)
{
	...
	if(strTestFileName != NULL){
		strcpy(strTestFileName,"/tmp/test.cfg");
		if((fp = fopen(strTestFileName, "wt")) != NULL){
			fprintf(fp, "1\n/tmp/123.txt\n");
			fclose(fp);
...
//		sprintf(buffer,"starttls disabled\n");
		...
		printf("/home/root/.esmtprc file open error!\n");
		SysDrvExit();
		exit(1);
	}
