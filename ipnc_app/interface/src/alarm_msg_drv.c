...
/* DMVA ALARM EVENT */
/**
* @brief DMVA ALARM EVENT START
*
* param Serial [I ] Serial Number of DMVA Currently it is 0
*/
void SendDMVAAlarmEventStart(int EvtType)
{
	ALARM_MSG_BUF msgbuf;
	...
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = 0;
	msgbuf.event = ALARM_EVENT_DMVA_START;
	msgbuf.serial = EvtType;
	msgsnd(qid, &msgbuf, sizeof(msgbuf)-sizeof(long), 0);/*send msg1*/
}

/**
* @brief DMVA ALARM EVENT STOP
*
*/
void SendDMVAAlarmEventStop()
{
	ALARM_MSG_BUF msgbuf;
	...
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = 0;
	msgbuf.event = ALARM_EVENT_DMVA_STOP;
	msgsnd(qid, &msgbuf, sizeof(msgbuf)-sizeof(long), 0);/*send msg1*/
}
