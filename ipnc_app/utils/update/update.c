...
#define KERNELBLOCK		"/dev/mtd3"
#define TEMP_PATH		"/tmp"
#define TEMP_FILE		"tmpfile.txt"
#ifdef IPNC_JFFS2_ON
#define FILESYS_PRE		"jffs2_"
#else
#define FILESYS_PRE		"ubifs_"
#endif
...
/* readbuff will be modified. */
static int update_bootargs(int flag, char* readbuff,int buf_sz)
{
	char mtd = flag ? '4': '6';
#ifndef IPNC_JFFS2_ON
	char target[] = "ubi.mtd=";
#else
	char target[] = "root=/dev/mtdblock";
#endif
	char *bootargs = readbuff + 4, *cmd;
	int env_size = buf_sz - 4, count = 0, length = 0, pos, t_size = strlen(target);
	if(readbuff == NULL || buf_sz <= 0)
		return FAIL;
	while(count < buf_sz){
		length = strlen(bootargs) + 1;
		if(!strncmp(bootargs, "bootargs=", 9))
			break;
		/* next parameter */
		bootargs += length;
		count += length;
	...
	for(count = pos = 0;pos < length;pos++){
		if(count == t_size){
			bootargs[pos] = mtd;
			break;
		}else if(bootargs[pos] == target[count])
			count++;
		else
			count = 0;
	}
	if(pos == length)
		return FAIL;
	/* there should be lots of space after bootargs */
	cmd = bootargs + length + 1;
	sprintf(cmd, "fw_setenv bootargs \"%s\"", bootargs + 9);
	system(cmd);
	return SUCCESS;
}
/**
...
//	char cmd[80];
	...
	fd = open("/dev/mtd2", O_RDONLY);
        ...
        size = lseek(fd, 0x0, SEEK_SET);
        if (size == -1)
        {
                DBG("=======lseek on mtd2 failed========\n");
                return FAIL;
        }
	...
	/* update the env only if writing to nand succeeds */
		...
		update_bootargs(flag, readbuff, size);
		sync();
//		reboot(RB_AUTOBOOT);
