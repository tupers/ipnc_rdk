...
int RecordToSD(char *strFileName)
{
	...
	if((fp = fopen(strFileName, "rt")) == NULL){
		FD_ERR("%s open fail, at %s, %d\n", strFileName, __FILE__, __LINE__);
		return -1;
	}
...
* @param bCfgFileOnly [I ] Only delete the configuration file or the source files to be deleted along.
* @return 0 on success.
*/
int RemoveFile(char *strFileName, int bCfgFileOnly)
{
	char cmd[80]= "\n";
	FILE *fp;
	int nFileCount, i, ret = 0;
...
void *FileDispatcherThr(void *arg)
{
	...
	if(pPrm->tFileType == ALL_TYPE){
		...
		RemoveFile(strFilePath, !pPrm->bRemoveSrc);
...
* @retval -1 Fail
*/
int DoFileDispatch(char *strFilePath, int nDestination, int bRemoveSrc)
{
	pthread_t* pTmpThr;
	...
	if(SemWait(gFileDispatcherSem) == 0){
		...
		if(gFileDispCount < FILE_DISP_THR_LIMIT){
			...
			if(strFilePath){
				...
				if(pFilePath == NULL){
					SemRelease(gFileDispatcherSem);
					RemoveFile(strFilePath, !bRemoveSrc);
			...
			if(pTmpThr == NULL){
				SemRelease(gFileDispatcherSem);
				if(pFilePath){
					RemoveFile(pFilePath, !bRemoveSrc);
					free(pFilePath);
				}
				FD_ERR("Not enough memory\n");
				return -1;
