...
int set_socket_timeout(int sd,unsigned int ms)
{
	struct timeval tm;
	unsigned s,us;
	if(ms > 0)
	{
		...
		setsockopt(sd,SOL_SOCKET,SO_SNDTIMEO,&tm,sizeof(tm));
		setsockopt(sd,SOL_SOCKET,SO_RCVTIMEO,&tm,sizeof(tm));
	}
