    ...
    if (frameList.numFrames)
    {
        ...
        for (frameId = 0; frameId < frameList.numFrames; frameId++)
        {
            ...
            if (pChObj->nextFid == pFrame->fid)
            {
				...
				/*
				 * If all the output queues for the channel are disabled then quickly return back
				 * the frames to previous link.
				 * This speeds up the chain. 
				 */	
				if((pChObj->enableOut[DEI_LINK_OUT_QUE_DEI_SC] == FALSE) &&
				   (pChObj->enableOut[DEI_LINK_OUT_QUE_VIP_SC] == FALSE) &&
				   (pChObj->enableOut[DEI_LINK_OUT_QUE_VIP_SC_SECONDARY_OUT] == FALSE))
				{
					doFrameDrop = TRUE;
				}	
                               ...
                               )
{
    ...
    inFrameList = &pReqObj->inFrameList;
    outFrameList = pReqObj->outFrameList;
...
Int32 DeiLink_drvGetProcessedData(DeiLink_Obj * pObj)
{
    ...
    DeiLink_drvReleaseFrames(pObj, pReqObj);
