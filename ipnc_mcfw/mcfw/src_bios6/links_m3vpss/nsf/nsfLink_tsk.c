    ...
    *pMsg = NULL;
    ...
    while (!runDone)
    {
        ...
        switch (cmd)
        {
            ...
            case NSF_LINK_CMD_ENABLE_SNF:
                NsfLink_drvEnableSNF(pObj, *(UInt32 *)Utils_msgGetPrm(pRunMsg));
                Utils_tskAckOrFreeMsg(pRunMsg, status);
                break;
				
			case NSF_LINK_CMD_ENABLE_TNF:
				NsfLink_drvEnableTNF(pObj, *(UInt32 *)Utils_msgGetPrm(pRunMsg));
				Utils_tskAckOrFreeMsg(pRunMsg, status);
				break;
			...
			case NSF_LINK_CMD_ENABLE_CHANNEL:
				channelNum = *((UInt32*)Utils_msgGetPrm(pRunMsg));
				NsfLink_enableChannel(pObj,channelNum);	
				Utils_tskAckOrFreeMsg(pRunMsg, status);
				break;
				
			case NSF_LINK_CMD_DISABLE_CHANNEL:
				channelNum = *((UInt32*)Utils_msgGetPrm(pRunMsg));
				NsfLink_disableChannel(pObj,channelNum);	
				Utils_tskAckOrFreeMsg(pRunMsg, status);
				break;				
