...
Int32 CaptureLink_drvCreateVideoDecoder(CaptureLink_Obj * pObj, UInt16 instId)
{
...
#ifdef SYSTEM_ENABLE_AUDIO
    {
        ...
        Vps_Tvp5158AudioModeParams audArgs;
    ...
    Vps_printf(" %d: CAPTURE: VIP %d: VID DEC %d (0x%02x): %04x:%04x:%04x, AUD_STATUS %d\n",
...
//                    if (instId == 0)
...
#ifdef  TI_816X_BUILD
                    {
                      ...
                      if (cpuRev < VPS_PLATFORM_CPU_REV_2_0)
                      {
                          ...
                          pObj->maxWidth = CAPTURE_DRV_MAX_WIDTH_LIMIT_TI816X_PG_1_1;
                      }
                    }
#endif
                }
    ...
    for (streamId = 0; streamId < CAPTURE_LINK_MAX_OUTPUT_PER_INST; streamId++)
    {
        ...
        if (frameList.numFrames)
        {
            for (frameId = 0; frameId < frameList.numFrames; frameId++)
            {
                ...
                pFrameInfo->ts64 = pAvSyncInfo_obj->timeInfo.wallTimeInTicks;
