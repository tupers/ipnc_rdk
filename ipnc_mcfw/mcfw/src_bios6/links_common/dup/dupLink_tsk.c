...
Int32 DupLink_drvCreate(DupLink_Obj * pObj, DupLink_CreateParams * pPrm)
{
	...
	pObj->syncDelayedOutputQueId = DUP_LINK_DELAYED_OUTPUT_QUE_INVALID_ID;
...
Int32 DupLink_setDelayOutQuePrms(DupLink_Obj * pObj,DupLink_DelayParams *delayPrms)
{
    ...
    if (status == 0)
    {
		...
		pObj->syncDelayedOutputQueId = delayPrms->syncOutQueId;
    }
                             ...
                             FVID2_FrameList * pFrameList)
{
    ...
    for (frameId = 0; frameId < pFrameList->numFrames; frameId++)
    {
		...
		if (pObj->syncDelayedOutputQueId != DUP_LINK_DELAYED_OUTPUT_QUE_INVALID_ID)
		{
			if(pObj->syncDelayedOutputQueId == queId)
			{
				/* Frame has been freed from sync out que Id.So put in delayed output queue now */
				DupLink_drvPutFramesInDelayOutQue(pObj,pOrgFrame);
			}
		}		
		else if ((pOrgFrameInfo->dupCount == 1) && (pObj->delayedOutputQueId <  pObj->createArgs.numOutQue))
			 {
