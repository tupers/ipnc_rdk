...
EncLink_ChCreateParams ENCLINK_DEFAULTCHCREATEPARAMS_MPEG4 = {
    .format = IVIDEO_MPEG4SP,
    .profile = NULL,
    .dataLayout = IVIDEO_PROGRESSIVE,
    .fieldMergeEncodeEnable = FALSE,
    .encodingPreset = ENC_LINK_DEFAULT_ALGPARAMS_ENCODINGPRESET,
    .enableAnalyticinfo = ENC_LINK_DEFAULT_ALGPARAMS_ANALYTICINFO,
    .enableWaterMarking = ENC_LINK_DEFAULT_ALGPARAMS_ENABLEWATERMARKING,
    ...
    .defaultDynamicParams = {
                             .intraFrameInterval =
                             ENC_LINK_DEFAULT_ALGPARAMS_INTRAFRAMEINTERVAL/2,
                             .targetBitRate =
                             ENC_LINK_DEFAULT_ALGPARAMS_TARGETBITRATE,
                             .interFrameInterval =
                             ENC_LINK_DEFAULT_ALGPARAMS_MAXINTERFRAMEINTERVAL,
                             .mvAccuracy =
                             ENC_LINK_DEFAULT_ALGPARAMS_MVACCURACY,
                             .inputFrameRate =
                             ENC_LINK_DEFAULT_ALGPARAMS_INPUTFRAMERATE/2
                             }
};
...
EncLink_ChCreateParams ENCLINK_DEFAULTCHCREATEPARAMS_JPEG = {
    ...
    .defaultDynamicParams =
    {
        ...
        .mvAccuracy         = ENC_LINK_DEFAULT_ALGPARAMS_MVACCURACY,
        .inputFrameRate     = ENC_LINK_DEFAULT_ALGPARAMS_INPUTFRAMERATE/12,
        .qpMin              = ENC_LINK_DEFAULT_ALGPARAMS_QPMIN,
        .qpMax              = ENC_LINK_DEFAULT_ALGPARAMS_QPMAX,
        .qpInit             = ENC_LINK_DEFAULT_ALGPARAMS_QPINIT,
    }
    ...
    switch (resClass)
    {
        ...
        case UTILS_ENCDEC_RESOLUTION_CLASS_5MP:
            ...
            *pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_5MP_HEIGHT;
            break;
        case UTILS_ENCDEC_RESOLUTION_CLASS_3MP:
            *pWidth = UTILS_ENCDEC_RESOLUTION_CLASS_3MP_WIDTH;
            *pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_3MP_HEIGHT;
            break;
...
static Int32 EncLink_codecCreateOutObj(EncLink_Obj * pObj)
{
    ...
    {
		//allocating memory space as ring buffer, and allocated size is half
	 	status= Utils_ringBuffer_init(&ringBuffer[i],  pOutObj->buf_size[i]*pOutObj->outNumBufs[i]);
        UTILS_assert(status == FVID2_SOK);

        for (bufIdx = 0; bufIdx < pOutObj->outNumBufs[i]; bufIdx++)
        {
            UTILS_assert((bufIdx + totalBufCnt) < ENC_LINK_MAX_OUT_FRAMES);
				      pOutObj->outBufs[bufIdx + totalBufCnt].addr = ringBuffer[i].baseAddr;
					pOutObj->outBufs[bufIdx + totalBufCnt].phyAddr = (UInt32)(ringBuffer[i].baseAddr);
					pOutObj->outBufs[bufIdx + totalBufCnt].bufSize = ringBuffer[i].totalSize;
                                                             ...
                                                             UInt32 chId)
{
	...
	if (pObj->createArgs.chCreateParams[chId].format == IVIDEO_MPEG4SP)
    {
        pChObj->algObj.algDynamicParams.targetFrameRate =
            ENC_LINK_DEFAULT_ALGPARAMS_TARGETFRAMERATEX1000/2;
        pChObj->algObj.algDynamicParams.refFrameRate =
            ENC_LINK_DEFAULT_ALGPARAMS_REFFRAMERATEX1000/2;
    }
    else
    {
...
static Int32 EncLink_codecCreateEncObj(EncLink_Obj * pObj, UInt32 chId)
{
    ...
    switch (pChObj->algObj.algCreateParams.format)
    {
        ...
        case IVIDEO_MPEG4SP:
            retVal = EncLinkMpeg4_algCreate(&pChObj->algObj.u.mpeg4AlgIfObj,
                                           &pChObj->algObj.algCreateParams,
                                           &pChObj->algObj.algDynamicParams,
                                           pObj->linkId, chId, scratchGroupID);
            break;
                                            ...
                                            UInt32 ivaID)
{
    ...
    switch (pReqObj->type)
    {
        ...
        case ENC_LINK_REQ_OBJECT_TYPE_DUMMY_CODEC_SWITCH:
        {
            ...
            UTILS_assert(pReqObj->OutBuf != NULL);
            UTILS_assert(pReqObj->OutBuf->channelNum < pObj->inQueInfo.numCh);
            pChObj = &pObj->chObj[pReqObj->OutBuf->channelNum];
            UTILS_assert (pChObj->switchCodec.switchCodecFlag == TRUE);
            pInChInfo = &pObj->inQueInfo.chInfo[pReqObj->OutBuf->channelNum];

			if (pInChInfo->scanFormat == FVID2_SF_INTERLACED)
			{
				pInChInfo->height *= 2;
			}
			pInChInfo->scanFormat = FVID2_SF_PROGRESSIVE;

            /* Switch codec Alg type if switchCodecFlag is enabled */
            EncLink_codecSwitchCodecAlg(pObj, pReqObj->OutBuf->channelNum);
            pChObj->switchCodec.switchCodecFlag = FALSE;
            ENCLINK_INFO_LOG(pObj->linkId,pReqObj->OutBuf->channelNum,
                             "Codec Switch: Complete [%d] ", ivaID);
            break;
        }
        default:
...
Int32 EncLink_codecCreate(EncLink_Obj * pObj, EncLink_CreateParams * pPrm)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        else if(pObj->createArgs.chCreateParams[chId].format == IVIDEO_MPEG4SP)
        {
            pObj->createArgs.chCreateParams[chId] = ENCLINK_DEFAULTCHCREATEPARAMS_MPEG4;
        }
    }
#endif
      ...
      while(numReqObjPerProcess < pChObj->numReqObjPerProcess) {
            ...
            (!Utils_queIsFull(&pObj->encProcessTsk[tskId].processQue)))
        {
            ...
            status = Utils_bitbufGetEmptyBuf(&pObj->outObj.bufOutQue,
                                             ...
                                             BIOS_NO_WAIT);
/*
	     if(Utils_ringBuffer_Size_Avail_for_MIN(&ringBuffer[pObj->outObj.ch2poolMap[chId]], pOutBuf->frameWidth*pOutBuf->frameHeight))
			{
				status1 = FVID2_SOK;
			}
	      else
...
Int32 EncLink_codecDelete(EncLink_Obj * pObj)
{
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
        ...
        switch (pChObj->algObj.algCreateParams.format)
        {
            ...
            case IVIDEO_MPEG4SP:
                EncLinkMpeg4_algDelete(&pChObj->algObj.u.mpeg4AlgIfObj);
                break;
    ...
    {
        {
            ...
            for (i = 0; i < pOutObj->numAllocPools; i++)
            {
    		Utils_ringBuffer_deinit(&ringBuffer[i]);
            }
        }
    }
                               ...
                               EncLink_ChSwitchCodecTypeParams * params)
{
    ...
    if (status == ENC_LINK_S_SUCCESS)
    {
        ...
        UTILS_assert(UTILS_ARRAYISVALIDENTRY(pReqObjCodecSwitch,pObj->encDummyReqObj.reqObjDummy));

        pObj->chObj[params->chId].dummyBitBuf.channelNum = params->chId;

        pReqObjCodecSwitch->type = ENC_LINK_REQ_OBJECT_TYPE_DUMMY_CODEC_SWITCH;
        pReqObjCodecSwitch->ivaSwitchSerializer = NULL;
        pReqObjCodecSwitch->OutBuf = &pObj->chObj[params->chId].dummyBitBuf;
        ivaId = pObj->ch2ProcessTskId[params->chId];
        ENCLINK_INFO_LOG(pObj->linkId,params->chId,"Queueing codec switch reqObj into IVA [%d]",ivaId);
        status = Utils_quePut(&pObj->encProcessTsk[ivaId].processQue,
                              pReqObjCodecSwitch, ti_sysbios_BIOS_NO_WAIT);
        UTILS_assert(status == FVID2_SOK);
    }
        ...
        if (rtParamUpdatePerFrame == TRUE)
        {
            switch (pChObj->algObj.algCreateParams.format)
            {
                ...
                case IVIDEO_MJPEG:
                    ...
                    break;
              case IVIDEO_MPEG4SP:
                    status = EncLinkMpeg4_algDynamicParamUpdate(
                                         &pChObj->algObj.u.mpeg4AlgIfObj,
                                         &pChObj->algObj.algCreateParams,
                                         &pChObj->algObj.algDynamicParams);
                    if (UTILS_ISERROR(status))
                    {
                                   ...
                                   EncLink_ReqBatch *pReqObjBatch)
{
  ...
  switch (pChObj->algObj.algCreateParams.format)
  {
      ...
      case IVIDEO_MPEG4SP:
          pReqObjBatch->codecSubmittedFlag = pReqObjBatch->codecSubmittedFlag |
                                              (0x1 <<
                                               ENC_LINK_GROUP_CODEC_CLASS_MPEG4);
          break;
      default:
          UTILS_assert(FALSE);
  }
  ...
  if (FVID2_SOK == status)
  {
    ...
    for ( i = 0; i < pReqObjBatch->numReqObjsInBatch; i++)
    {
      Vps_printf ("ENC : IVAHDID : %d %d\n", tskId,
                  pReqObjBatch->pReqObj[i]->OutBuf->channelNum);
    }
                                  ...
                                  EncLink_ReqBatch *pReqObjBatch)
{
  ...
  for (i = 0; i < pReqObjBatch->numReqObjsInBatch; i++)
  {
    ...
    switch (format)
    {
        ...
        case IVIDEO_MPEG4SP:
            status = EncLinkMpeg4_algSetConfig(&pChObj->algObj);
            status = Enclink_mpeg4EncodeFrame(pChObj, pReqObj);
            if (UTILS_ISERROR(status))
            {
                UTILS_warn("ENCLINK:ERROR in "
                           "Enclink_h264EncodeFrame.Status[%d]",status);
...
static Int32 EncLink_codecSwitchCodecAlg(EncLink_Obj * pObj, UInt32 chId)
{
    ...
    if (!UTILS_ISERROR(status))
    {
        status = System_sendLinkCmd(pObj->linkId, ENC_LINK_CMD_LATE_ACK);
    }
