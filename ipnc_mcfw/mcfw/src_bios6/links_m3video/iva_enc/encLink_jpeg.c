...
static JPEGVENC_Handle enc_link_jpeg_create(const IJPEGVENC_Fxns * fxns,
                                            ...
                                            Int scratchGroupID);
static Void enc_link_jpeg_delete(Int scratchGroupID,JPEGVENC_Handle handle);
...
static JPEGVENC_Handle enc_link_jpeg_create(const IJPEGVENC_Fxns * fxns,
                                            ...
                                            Int scratchGroupID)
{
    return ((JPEGVENC_Handle) ALG_create(scratchGroupID,(IALG_Fxns *) fxns,
                                      ...
                                      algDynamicParams)
{
    ...
    /*------------------------------------------------------------------------*/
	if (algCreateParams->inputChromaFormat == XDM_YUV_422ILE)
		inputBufDesc->numPlanes = 1;
	else
		inputBufDesc->numPlanes = 2;
    ...
    else
    {
		...
		/* work around for YUV 422 input, need to remove it after codec update  */
		if (algCreateParams->inputChromaFormat == XDM_YUV_422ILE)
			algObj->dynamicParams.videnc2DynamicParams.captureWidth = algObj->dynamicParams.videnc2DynamicParams.captureWidth/2;
    }
    if (algCreateParams->tilerEnable)
    {
...
static Void enclink_jpeg_freersrc(EncLink_JPEGObj * hObj, Int rsrcMask)
{
    ...
    {
        enc_link_jpeg_delete(hObj->scratchID,hObj->algHandle);
        hObj->algHandle = NULL;
    }
}
...
Int32 EncLinkJPEG_algSetConfig(EncLink_algObj * algObj)
{
    ...
    if ((bitMask >> ENC_LINK_SETCONFIG_BITMASK_RCALGO) & 0x1)
    {
        ...
        {
        	algObj->algDynamicParams.rcAlg = IJPEGVENC_RATECONTROL_VBR_STORAGE;
        ...
        {
        	algObj->algDynamicParams.rcAlg = IJPEGVENC_RATECONTROL_CBR_LOW_DELAY;
		  	algObj->u.jpegAlgIfObj.dynamicParams.rateControlParams.rcAlgo =
    ...
    if (setConfigFlag)
    {
        ...
        {
            UTILS_warn("\n JPGLINK: ERROR in Run time parameters changes, Extended Error code:%d \n", \
