...
#include <mcfw/src_bios6/links_m3video/codec_utils/utils_encdec.h>
#ifdef TI_816X_BUILD
#define IRESMAN_HDVICP2_EARLYACQUIRE_HDVICP2_NUMRESOURCES                  (3)
#endif
#if defined(TI_814X_BUILD) || defined(TI_8107_BUILD)
#define IRESMAN_HDVICP2_EARLYACQUIRE_HDVICP2_NUMRESOURCES                  (1)
#endif

#define IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN                          (32)
#define IRESMAN_HDVICP2_EARLYACQUIRE_INVALID_OWNERID                     (~(0u))

typedef enum IRESMAN_HDVICP2_EARLYACQUIRE_ResBusyStatus_e {
    IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE,
    IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_INUSE,
} IRESMAN_HDVICP2_EARLYACQUIRE_ResBusyStatus_e;

/*
 * Holds the internal state of an initialized object
 */
typedef struct IRESMAN_HDVICP2_EarlyAcquireInternalState {

    Bool initDone;                             /** Number of instances of this
                                                 * resource manager
                                                 */
    ...
    struct IRESMAN_HDVICP2_ResourceInternalState_s {
        volatile IRESMAN_HDVICP2_EARLYACQUIRE_ResBusyStatus_e
            resourceBusyStatus;
                                                    /**< Busy-status of HDVICP2
                                                     * resources.
                                                     */
        UInt32 resourceOwnerId;

        Utils_QueHandle freeSemQ;                  /**< Queue for holding Free
                                                     * semaphore handles */
        Semaphore_Handle freeSemQMem[IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN];

        Semaphore_Struct
            pendSemStruct[IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN];

        Semaphore_Handle
            pendSemHandle[IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN];

        Utils_QueHandle pendQ;                     /**< Resource acquire pendQ */

        Semaphore_Handle pendQMem[IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN];
    } res[IRESMAN_HDVICP2_EARLYACQUIRE_HDVICP2_NUMRESOURCES];

} IRESMAN_HDVICP2_EarlyAcquireInternalState;

IRESMAN_HDVICP2_EarlyAcquireInternalState g_HDVICPEarlyAcquireState =
    {.initDone = FALSE };

static
XDAS_Void IRESMAN_HDVICP2_EarlyAcquireInitResource(struct
                                                   IRESMAN_HDVICP2_ResourceInternalState_s
                                                   *resObj)
{
    Int i;
    Semaphore_Params semParams;
    ...
    resObj->resourceBusyStatus =
        IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE;
    resObj->resourceOwnerId = IRESMAN_HDVICP2_EARLYACQUIRE_INVALID_OWNERID;
    status = Utils_queCreate(&resObj->freeSemQ,
                             UTILS_ARRAYSIZE(resObj->freeSemQMem),
                             resObj->freeSemQMem, 0);
    UTILS_assert(UTILS_ISERROR(status) == FALSE);
    status = Utils_queCreate(&resObj->pendQ,
                             UTILS_ARRAYSIZE(resObj->pendQMem),
                             resObj->pendQMem, 0);
    UTILS_assert(UTILS_ISERROR(status) == FALSE);

    for (i = 0; i < IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN; i++)
    {
        Semaphore_Params_init(&semParams);
        Semaphore_construct(&resObj->pendSemStruct[i], 0, &semParams);
        resObj->pendSemHandle[i] = Semaphore_handle(&resObj->pendSemStruct[i]);
        status = Utils_quePut(&resObj->freeSemQ,
                              resObj->pendSemHandle[i], BIOS_NO_WAIT);
        UTILS_assert(UTILS_ISERROR(status) == FALSE);
    }
}

static
XDAS_Void IRESMAN_HDVICP2_EarlyAcquireDeInitResource(struct
                                                     IRESMAN_HDVICP2_ResourceInternalState_s
                                                     *resObj)
{
    Int i;
    ...
    resObj->resourceBusyStatus =
        IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE;
    resObj->resourceOwnerId = IRESMAN_HDVICP2_EARLYACQUIRE_INVALID_OWNERID;
    status = Utils_queDelete(&resObj->freeSemQ);
    UTILS_assert(UTILS_ISERROR(status) == FALSE);
    status = Utils_queDelete(&resObj->pendQ);
    UTILS_assert(UTILS_ISERROR(status) == FALSE);

    for (i = 0; i < IRESMAN_HDVICP2_EARLYACQUIRE_MAX_PENDQLEN; i++)
    {
        Semaphore_destruct(&resObj->pendSemStruct[i]);
        resObj->pendSemHandle[i] = NULL;
    }
}
...
{
    UInt hwiKey;
    IRESMAN_HDVICP2_EarlyAcquireInternalState *resmanInternalState =
        &g_HDVICPEarlyAcquireState;

    hwiKey = Hwi_disable();
    if (resmanInternalState->initDone == FALSE)
    {
        Int i;

        resmanInternalState->numResources = HDVICP2_GetNumberOfIVAs();
        UTILS_assert(resmanInternalState->numResources <=
                     IRESMAN_HDVICP2_EARLYACQUIRE_HDVICP2_NUMRESOURCES);
        for (i = 0; i < resmanInternalState->numResources; i++)
        {
            IRESMAN_HDVICP2_EarlyAcquireInitResource(&resmanInternalState->
                                                     res[i]);
        }
        resmanInternalState->initDone = TRUE;
    }
    Hwi_restore(hwiKey);
...
XDAS_Void IRESMAN_HDVICP2_EarlyAcquireDeInit()
{
    ...
    hwiKey = Hwi_disable();
    if (resmanInternalState->initDone == TRUE)
    {
        Int i;

        for (i = 0; i < resmanInternalState->numResources; i++)
        {
            IRESMAN_HDVICP2_EarlyAcquireDeInitResource(&resmanInternalState->
                                                       res[i]);
        }
        resmanInternalState->numResources = 0;
        resmanInternalState->initDone = FALSE;
    }
    Hwi_restore(hwiKey);
...
/*
 *  ========  IRESMAN_HDVICP2_EarlyAcquire ========
 *  Function called by the algorithm to do a JEarlyIT acquire of the IVA-HD resource
                                       ...
                                       XDAS_UInt32 ivaChID)
{
    UInt32 resId;
    UInt hwiKey;
    Semaphore_Handle pendSem = NULL;
    Bool doWaitOnFreeResource = FALSE;
    IRESMAN_HDVICP2_EarlyAcquireInternalState *resmanInternalState =
        &g_HDVICPEarlyAcquireState;
    struct IRESMAN_HDVICP2_ResourceInternalState_s *resObj = NULL;
    Int32 status;

    UTILS_assert(resmanInternalState->initDone == TRUE);
    hwiKey = Hwi_disable();

    resId = Utils_encdecMapIVACh2IVAResId(ivaChID);

    UTILS_assert(resId < resmanInternalState->numResources);

    resObj = &resmanInternalState->res[resId];

    if (Utils_queIsEmpty(&resObj->pendQ)
        &&
        Utils_queIsFull(&resObj->freeSemQ)
        &&
        (resObj->resourceBusyStatus ==
         IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE))
    {
        /* IVA Resource is free */
        doWaitOnFreeResource = FALSE;
        resObj->resourceBusyStatus =
            IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_INUSE;
        resObj->resourceOwnerId = ivaChID;
    }
    else
    {
        status =
            Utils_queGet(&resObj->freeSemQ, (Ptr *) & pendSem, 1, BIOS_NO_WAIT);
        UTILS_assert(!UTILS_ISERROR(status));
        status = Utils_quePut(&resObj->pendQ, pendSem, BIOS_NO_WAIT);
        UTILS_assert(!UTILS_ISERROR(status));
        doWaitOnFreeResource = TRUE;
    }
    Hwi_restore(hwiKey);

    if (doWaitOnFreeResource)
    {
        UTILS_assert(pendSem != NULL);
        Semaphore_pend(pendSem, BIOS_WAIT_FOREVER);
        hwiKey = Hwi_disable();
        UTILS_assert(resObj->resourceBusyStatus ==
                     IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE);
        resObj->resourceBusyStatus =
            IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_INUSE;
        resObj->resourceOwnerId = ivaChID;
        status = Utils_quePut(&resObj->freeSemQ, pendSem, BIOS_NO_WAIT);
        UTILS_assert(!UTILS_ISERROR(status));
        Hwi_restore(hwiKey);
    }
    return;
                                       ...
                                       XDAS_UInt32 ivaChID)
{
    ...
    UTILS_assert(resObj->resourceBusyStatus ==
                 IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_INUSE);
    UTILS_assert(resObj->resourceOwnerId == ivaChID);

    resObj->resourceBusyStatus =
        IRESMAN_HDVICP2_EARLYACQUIRE_RESBUSYSTATUS_FREE;
    resObj->resourceOwnerId = IRESMAN_HDVICP2_EARLYACQUIRE_INVALID_OWNERID;

    status = Utils_queGet(&resObj->pendQ, (Ptr *) & pendSem, 1, BIOS_NO_WAIT);
    if (!UTILS_ISERROR(status))
    {
        UTILS_assert(pendSem != NULL);
        Semaphore_post(pendSem);
    }
