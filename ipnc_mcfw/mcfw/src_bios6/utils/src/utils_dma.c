...
void Utils_dmaCallback_new(unsigned int tcc, EDMA3_RM_TccStatus status, void *appData)
{
    (void)tcc;
    Utils_DmaChObj *pObj = (Utils_DmaChObj *)appData;
    EDMA3_DRV_Result edma3Result = EDMA3_DRV_SOK;
...
Int32 Utils_dmaCreateCh_new(Utils_DmaChObj *pObj, UInt32 eventQ)
{
...
Int32 Utils_dmaDeleteCh_new(Utils_DmaChObj *pObj)
{
    EDMA3_DRV_Result edma3Result = EDMA3_DRV_SOK;
...
Int32 Utils_dmaTrigger_new(Utils_DmaChObj *pObj)
{
    EDMA3_DRV_Result edma3Result = EDMA3_DRV_SOK;
    uint16_t tccStatus;
    ...
    if (edma3Result != EDMA3_DRV_SOK)
    {
        Vps_printf(" UTILS: DMA: Utils_dmaTrigger() ... FAILED (%d) \n",edma3Result);
    }

    return edma3Result;
}

Int32 Utils_dmaFill3D(Utils_DmaChObj *pObj, Utils_DmaCopy3D *pInfo)
{
    EDMA3_DRV_Result edma3Result = EDMA3_DRV_SOK;
	...
	Vps_printf("dstaddr is [0x%x],srcaddr is [0x%x]\n",(UInt32)pInfo->destAddr,(UInt32)pInfo->srcAddr);
	Vps_printf("aCnt is [%d],bcnt is [%d],ccnt is [%d]\n",pInfo->acnt,pInfo->bcnt,pInfo->ccnt);
	Vps_printf("srcBIdx is [%d],destBIdx is [%d],srcCIdx is [%d],destCIdx is [%d]\n",pInfo->srcBidx,pInfo->dstBidx,pInfo->srcCidx,pInfo->dstCidx);
	
	pParamSet = &pObj->pParamSet[0];
	
    pParamSet->destAddr   = (UInt32)pInfo->destAddr;
    pParamSet->srcAddr    = (UInt32)pInfo->srcAddr;

    pParamSet->srcBIdx    = pInfo->srcBidx;
    pParamSet->destBIdx   = pInfo->dstBidx;
    pParamSet->srcCIdx    = pInfo->srcCidx;
    pParamSet->destCIdx   = pInfo->dstCidx;

    pParamSet->aCnt       = pInfo->acnt;
    pParamSet->bCnt       = pInfo->bcnt;
    pParamSet->cCnt       = pInfo->ccnt;

    pParamSet->bCntReload = pInfo->bcnt;
    ...
    byteToWrite = (pParamSet->aCnt)*(pParamSet->bCnt)*(pParamSet->cCnt);
        
    Cache_wb((Ptr)pParamSet->srcAddr,byteToWrite,TRUE,NULL);

    edma3Result = Utils_dmaTrigger_new(pObj);
