                                  ...
                                  Int32 Padding, Int32 BoxCarDataSize)
{
	...
	if(pFormat->dataFormat == (FVID2_DF_YUV422I_UYVY | FVID2_DF_YUV420SP_UV))
	{
		/* Y plane */
		size = pFormat->pitch[0] * pFormat->height;
		
		/* C plane starts at the end of Y plane */
		cOffset = size;
		
		/* C plane */
		size += pFormat->pitch[0] * pFormat->height/2;
