#ifndef EZ_ALG_XDAIS_H_
#define EZ_ALG_XDAIS_H_
...
#define ALG_DEFMEMRECS 4   /**< Default number of memory records. */
#define ALG_OBJMEMREC  0   /**< Memory record index of instance object. */
#define ALG_SYSCMD     256 /**< Minimum "system" ALG_Cmd value. */

#define ALG_EOK        0   /**< Successful return status code. */
#define ALG_EFAIL      -1  /**< Unspecified error return status code. */

#define ALG_CUSTOMFAILBASE  -2048/**< Algorithm-specific failure code end.
                                   *
                                   *   @remarks This is 0xfffff800.
                                   */
#define ALG_CUSTOMFAILEND -256   /**< Algorithm-specific failure code base.
                                   *
 ...
 */
typedef enum ALG_MemAttrs {
    ALG_SCRATCH,           /**< Scratch memory. */
    ALG_PERSIST,           /**< Persistent memory. */
    ALG_WRITEONCE          /**< Write-once persistent memory. */
} ALG_MemAttrs;

#define ALG_MPROG  0x0008  /**< Program memory space bit. */
#define ALG_MXTRN  0x0010  /**< External memory space bit. */
 ...
 *  ======== ALG_MemSpace ========
 */
typedef enum ALG_MemSpace {
    ALG_EPROG =            /**< External program memory */
        ALG_MPROG | ALG_MXTRN,

    ALG_IPROG =            /**< Internal program memory */
        ALG_MPROG,

    ALG_ESDATA =           /**< Off-chip data memory (accessed sequentially) */
        ALG_MXTRN + 0,

    ALG_EXTERNAL =         /**< Off-chip data memory (accessed randomly) */
        ALG_MXTRN + 1,

    ALG_DARAM0 = 0,        /**< Dual access on-chip data memory */
    ALG_DARAM1 = 1,        /**< Block 1, if independant blocks required */

    ALG_SARAM  = 2,        /**< Single access on-chip data memory */
    ALG_SARAM0 = 2,        /**< Block 0, equivalent to ALG_SARAM */
    ALG_SARAM1 = 3,        /**< Block 1, if independant blocks required */

    ALG_DARAM2 = 4,        /**< Block 2, if a 3rd independent block required */
    ALG_SARAM2 = 5         /**< Block 2, if a 3rd independent block required */
} ALG_MemSpace;
 ...
 */
typedef struct ALG_MemRec {
    unsigned int             size;       /**< Size in MAU of allocation */
    int             alignment;  /**< Alignment requirement (MAU) */
    ALG_MemSpace   space;      /**< Allocation space */
    ALG_MemAttrs   attrs;      /**< Memory attributes */
...
typedef struct ALG_Obj {
    struct ALG_Fxns *fxns;     /**< Pointer to ALG function table. */
} ALG_Obj;
 ...
 */
typedef struct ALG_Obj *ALG_Handle;
 ...
 *
 *  @remarks    This command is used in conjunction with ALG_Status to get
 *              and set algorithm specific attributes via the algControl()
 *               method.
 */
typedef unsigned int ALG_Cmd;
 ...
 *                          then #ALG_DEFMEMRECS.
 *		algProcess()	  - Process Alg
 */
typedef struct ALG_Fxns {
/**
 ...
 *              ALG_Fxns) must be set to @c NULL.  This is equivalent to
 *              the following implementation:
 *  @code
 *      void algActivate(ALG_Handle handle)
 *      {
 *      }
 *  @endcode
 *
 ...
 *              of initialized memory records is #ALG_DEFMEMRECS.
 *
 ...
 *
 *  @post       For each memory descriptor in @c memTab with an #ALG_PERSIST or
 *              #ALG_SCRATCH attribute, the algorithm does not set its base
 *              field.
 *
 *  @post       @c memTab[0] defines the memory required for the instance's
 *              object and this object's first field is an ALG_Obj structure.
 *
 *  @post       @c memTab[0] is requested as persistent memory.
 *
 *  @sa         algFree()
 */
    int     (*algAlloc)(int NummemTab, ALG_MemRec *memTab);
 ...
 *              the module's static function table (of type ALG_Fxns) must
 *              be set to @c NULL.  This is equivalent to the following
 *              implementation:
 *  @code
 *      void algControl(ALG_Handle handle, ALG_Cmd cmd, ALG_Status *status)
 *      {
 *          return (ALG_EFAIL);
 *      }
 *  @endcode
 ...
 *              return value is not equal to ALG_EOK.
 *
 *  @sa         algInit()
 */
    int     (*algControl)(ALG_Handle handle, ALG_Cmd cmd,void * InArgs,void * OutArgs);
 ...
 *              (of type ALG_Fxns) must be set to @c NULL.  This is
 *              equivalent to the following implementation:
 *  @code
 *      void algDeactivate(ALG_Handle handle)
 *      {
 *      }
 *  @endcode
 *
 ...
 *  @sa         algAlloc()
 */
    int     (*algFree)(ALG_Handle handle,int NummemRec, ALG_MemRec *memTab);
 ...
 *                              to an initialized ALG_Obj structure.  Its
 ...
 *
 *  @remarks    @c handle is a pointer to an initialized ALG_Obj structure.
 ...
 *
 *  @remarks    The client is not required to satisfy the ALG_MemSpace
 ...
 *
 *  @remarks    The client may allocate the requested #ALG_WRITEONCE
 ...
 *      } EncoderObj;
 *
 *      int algInit(ALG_Handle handle, ALG_MemRec memTab[],
 *              ALG_Handle parent, ALG_Params *params)
 *      {
 ...
 *          }
 *
 *          if (ALG_isOffChip(memTab[1].space)) {
 ...
 *              appropriate ALG_Fxns structure.
 *
 ...
 *              base address to an #ALG_WRITEONCE @c memTab entry,
 ...
 *              address to an #ALG_WRITEONCE @c memTab[] entry, the
 ...
 *              client may treat #ALG_WRITEONCE buffers in the same
 *              way as #ALG_PERSIST buffers; by allocating new memory
 *              and granting it to the algorithm using the base field.
 *
 *  @pre        The buffer pointed to in @c memTab[0] is initialized
 *              with a pointer to the appropriate ALG_Fxns structure
 *              followed by all 0s.
 *
 ...
 *
 *  @post       All subsequent memory accesses to the #ALG_WRITEONCE
 ...
 *
 *  @retval     #ALG_EOK       @copydoc ALG_EOK
 *  @retval     #ALG_EFAIL     @copydoc ALG_EFAIL
 ...
 *  @sa         algMoved()
 */
    int     (*algInit)(ALG_Handle handle, int NummemRec,const ALG_MemRec *memTab);
 ...
 *              in the module's static function table (of type ALG_Fxns) must
 *              be set to @c NULL.  This is equivalent to asserting that the
 *              algorithm's instance objects cannot be moved.
 */
    void    (*algMoved)(ALG_Handle handle, const ALG_MemRec *memTab,
 ...
 *              to be #ALG_DEFMEMRECS.  This is equivalent to the following
 *              implementation:
 *  @code
 *      void algNumAlloc(void)
 *      {
 *          return (ALG_DEFNUMRECS);
 *      }
 *  @endcode
 *
 *  @remarks    If a module does not implement this method, the algNumAlloc()
 *              field in the module's static function table (of type ALG_Fxns)
 ...
 *      }
 *
 *      int algAlloc(const ALG_Params *p, struct ALG_Fxns **pFxns,
 *              ALG_MemRec memTab)
 *      {
 *          int n;
 *
 ...
 */
    ...
    int		(*algProcess)(ALG_Handle handle,void *InArgs,void *Result);

    int		(*algdma_actcbx)(char * dst,char *  src,unsigned int width,unsigned int height,unsigned src_pitch,unsigned dst_pitch,unsigned int cacheValid);
} ALG_Fxns;
