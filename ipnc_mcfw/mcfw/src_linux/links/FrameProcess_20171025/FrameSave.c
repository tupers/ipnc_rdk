...
int MkErrImgSaveDir(AlgSaveImg info,char *pathname)
{
	...
	sprintf(dirName, "%s/%s/%s_%d",IMGSAVE_ROOT,info.Alg_saveImgdirName,IMGSAVE_DIR,info.Alg_saveImgIndex);
	OSA_mkdir(dirName);	

	sprintf(dirName, "%s/%s_%d",info.Alg_saveImgdirName,IMGSAVE_DIR,info.Alg_saveImgIndex);
...
Int32 FrameSave_Process(FrameProcessLink_Obj * pObj)
{
	...
	while(1)
	{
		...
		{	
			...
			if(ret < 0)
			{
				printf("%s in %d : WriteImgFile fail\n",__func__,__LINE__);
			}																
		}
