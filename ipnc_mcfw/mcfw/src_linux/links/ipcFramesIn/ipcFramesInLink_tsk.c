...
Int32 IpcFramesInLink_processFrameBufs(IpcFramesInLink_Obj * pObj)
{
    ...
    if (numFrameBufs)
    {
    	OSA_printf("IPC Frame In has frame buf\n");
        if (pObj->createArgs.cbFxn)
        {
        	OSA_printf("IPC Frame In call cbfxn\n");
	...
	if (numFrameBufs && pObj->createArgs.baseCreateParams.notifyNextLink)
    {
        ...
        System_sendLinkCmd(pObj->createArgs.baseCreateParams.outQueParams[0].nextLink, SYSTEM_CMD_NEW_DATA);
    }
...
Int32 IpcFramesInLink_getFullFrames(OSA_TskHndl * pTsk, UInt16 queId,
...
Int32 IpcFramesInLink_putEmptyFrames(OSA_TskHndl * pTsk, UInt16 queId,
                                   VIDFrame_BufList * pFrameBufList)
{
    UInt32 bufId;
    Int32 status;
                                         ...
                                         VIDFrame_BufList *bufList)
{
    ...
    bufList->numFrames = 0;
    status = IpcFramesInLink_getFullFrames(pTsk,0,bufList);
    return status;
}
                                          ...
                                          VIDFrame_BufList *bufList)
{
    ...
    status = IpcFramesInLink_putEmptyFrames(pTsk,0,bufList);
    return status;
}
