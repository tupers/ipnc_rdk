...
Void IpcFramesOutLink_setOutQueInfo(IpcFramesOutLink_Obj *pObj)
{
	...
	OSA_printf("prevLinkQueId is [%d],numQue is [%d]\n",pObj->createArgs.baseCreateParams.inQueParams.prevLinkQueId,pObj->inQueInfo.numQue);
    /* get input queue info */
    OSA_assert(pObj->createArgs.baseCreateParams.inQueParams.prevLinkQueId<pObj->inQueInfo.numQue);

    pInQueInfo =&pObj->inQueInfo.queInfo[pObj->createArgs.baseCreateParams.inQueParams.prevLinkQueId];
...
Int32 IpcFramesOutLink_processFrameBufs(IpcFramesOutLink_Obj * pObj)
{
    ...
    pInQueParams = &pObj->createArgs.baseCreateParams.inQueParams;
	System_getLinksFullFrames(pInQueParams->prevLinkId,pInQueParams->prevLinkQueId, &bufList);
...
Int32 IpcFramesOutLink_releaseFrameBufs(IpcFramesOutLink_Obj * pObj)
{
    ...
    do
    {
        ...
        if (pListElem != NULL)
        {
            ...
            SYSTEM_IPC_FRAMES_SET_BUFSTATE(pListElem->bufState,IPC_FRAMEBUF_STATE_FREE);
                                         
        	status = OSA_quePut(&pObj->listElemQue,(Int32)pListElem,OSA_TIMEOUT_NONE);
        	OSA_assert(status == OSA_SOK);
        }
