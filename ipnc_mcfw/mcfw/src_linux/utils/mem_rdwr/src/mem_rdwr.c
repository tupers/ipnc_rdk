...
{
  OSA_printf("\n");
  OSA_printf(" Memory read/write utility, (c) Texas Instruments 2010\n");
  OSA_printf(" Built on %s %s\n", __DATE__, __TIME__);  
  OSA_printf("\n");  
  OSA_printf(" MEM WRITE, %s --wr <memory physical address in Hex> <memory value to Write in Hex>\n", str);
  OSA_printf("\n");
  OSA_printf(" MEM READ , %s --rd <memory physical address in Hex> <memory size in 32-bit words in Dec> --cols <number of coloums of print in Dec>\n", str);
  OSA_printf("\n");
  OSA_printf(" LOAD FILE, %s --fload <memory physical address in Hex> <filename>\n", str);
  OSA_printf("\n");
  OSA_printf(" SAVE FILE, %s --fsave <memory physical address in Hex> <memory size in Hex in bytes> <filename>\n", str);
  OSA_printf("\n");
  OSA_printf(" Physical memory address MUST be 4byte aligned\n");
  OSA_printf("\n");  
  OSA_printf(" Example,\n");
  OSA_printf(" MEM WRITE, %s --wr 4810D000 1234abcd \n", str);
  OSA_printf(" MEM READ , %s --rd 4810D000 16 \n", str);
  OSA_printf(" MEM READ , %s --rd 4810D000 16 --cols 1 \n", str);
  OSA_printf(" FILE SAVE, %s --fsave 81000000 0x100 test.bin \n", str); 
  OSA_printf(" FILE LOAD, %s --fload 81000000 test.bin \n", str);   
  OSA_printf("\n");
...
int MEM_RDWR_readMem()
{
  ...
  while(memSize) {
    OSA_printf(" 0x%08x:", memPhysAddr);  
    for(i=0; i<numPrintCols; i++) {
    
      OSA_printf(" %08X", *memVirtAddr);  
