...
static int Ftp_Putfile(char *pathname,unsigned int fileSize,int port,char *ip)
{
	...
	printf("pathname:%s,file size:%d,port:%d,ip:%s\n",pathname,fileSize,port,ip);
...
static void *Ftp_CtrlThread(void *arg)
{
	...
	if(ret != 0)
	{
		printf("%s in %d:Ftp_thrCreate fail\n",__func__,__LINE__);
