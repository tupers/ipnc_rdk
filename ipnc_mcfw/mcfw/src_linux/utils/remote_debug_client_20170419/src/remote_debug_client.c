...
static void *RemoteMsgThread(void *arg)
{
	...
	gRemoteDebug_clientObj.qid = RemoteDebug_Msg_Init(REMOTEDEBUG_KEY);
	if(gRemoteDebug_clientObj.qid < 0)
	{
		printf("%s in %d:RemoteDebug_Msg_Init fail\n",__func__,__LINE__);	

		return NULL;	
	}
	gRemoteDebug_clientObj.mid = OSA_ShareMemInit(REMOTEDEBUG_KEY,PROC_MEM_SIZE*MAX_SHARE_PROC);
	if(gRemoteDebug_clientObj.mid < 0)
	{
		printf("%s in %d:OSA_ShareMemInit fail\n",__func__,__LINE__);	

		return NULL;	
	}		
	while(tQuit == 0)
	{
		msg_size = RemoteDebug_Msg_Rsv( gRemoteDebug_clientObj.qid, &msgbuf, sizeof(msgbuf),
				MSG_TYPE_MSG1);
		...
		else 
		{
			switch(msgbuf.cmd)
			{
				...
				case REMOTEDEBUG_MSG_SOCKET:
				{
					...
					gRemoteDebug_clientObj.sClient.remote_addr.sin_port = htons(msgbuf.port);
					inet_aton(msgbuf.ip, &(gRemoteDebug_clientObj.sClient.remote_addr.sin_addr));
				...
				{
					if(msgbuf.shmsize > REMOTE_DEBUG_LINE_BUF_SIZE)
						break;
					...
					pFreebuf->size = msgbuf.shmsize;
					
					OSA_ShareMemRead(gRemoteDebug_clientObj.mid,0,pFreebuf->data, pFreebuf->size);
			...
			if(msgbuf.src != 0){
				...
				msgsnd(gRemoteDebug_clientObj.qid,&msgbuf,sizeof(msgbuf)-sizeof(long),0);
			}
		}		
	}
	return NULL;
