...
Int FirmwareLoad_ipcStop(UInt16 procId)
{
    ...
    status = Ipc_control (procId,
                          ...
                          NULL);
    OSA_printf ("After Ipc_stopCallback status: [0x%08x]\n",
                 status);
...
Int FirmwareLoad_ipcLoad(UInt16 procId)
{
    ...
    status = Ipc_control (procId,
                          ...
                          NULL);
    if (status < 0) {
        OSA_printf ("Error in Ipc_control "
                     "Ipc_CONTROLCMD_LOADCALLBACK [0x%08x]\n",
                      status);
    }
    else {
        OSA_printf ("After Ipc_loadcallback status [0x%08x]\n",
                     status);
    }
...
Int FirmwareLoad_ipcStart(UInt16 procId)
{
    ...
    if (status >= 0) {
        ...
        if (status < 0) {
            ...
            OSA_printf ("After Ipc_startcallback status [0x%08x]\n",
                         status);
        }
    }
...
Int FirmwareLoad_startup (UInt16 procId, String filePath)
{
    ...
    if (status >= 0) {
        ...
        if (status < 0)
            OSA_printf ("ProcMgr_attach failed [0x%x]\n", status);
        else
            OSA_printf ("Attached to slave procId %d.\n", procId);
        ...
        if(status >= 0)
        {
            ...
            status = ProcMgr_load (handle,
                                   ...
                                   &fileId);
            if (status < 0)
                OSA_printf ("Error in ProcMgr_load [0x%x]\n", status);
            else
                OSA_printf ("Loaded file %s on slave procId %d.\n", filePath, procId);
        }
        ...
        if (status >= 0)
        {
            ...
            if (status < 0)
                OSA_printf ("ProcMgr_start failed [0x%x]\n", status);
            else
                OSA_printf ("Started slave procId %d.\n", procId);
        }
...
Int FirmwareLoad_shutdown (UInt16 procId)
{
    ...
    if (status >= 0) {
        ...
        OSA_printf ("Stopped slave procId %d.\n", procId);

        fileId = ProcMgr_getLoadedFileId (handle);
        status = ProcMgr_unload (handle, fileId) ;

        OSA_printf ("Unloaded slave procId %d.\n", procId);
        status = ProcMgr_detach (handle);

        OSA_printf ("Detached from slave procId %d.\n", procId);
...
Void FirmwareLoad_printStatus (Void)
{
    ...
    OSA_printf ("Current status of slave cores:\n");
