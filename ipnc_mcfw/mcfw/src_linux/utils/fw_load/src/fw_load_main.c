...
int main (int argc, char ** argv)
{
    ...
    if ((argc == 2) && (strcmp (argv [1], "--help") == 0)) {
        ...
        if (strcmp (argv [1], "startup") == 0)
        {
            if (argc != 4) {
                status = -1;
                OSA_printf("[File path] and <Core name> need to be provided"
                            " for 'startup' !\n");
                FirmwareLoad_printUsageInfo ();
            }
        ...
        if (strcmp (argv [1], "shutdown")  == 0)
        {
            if (argc < 3) {
                status = -1;
                OSA_printf("<Core name> needs to be provided for"
                            " 'shutdown' !\n");
                FirmwareLoad_printUsageInfo ();
            }
        ...
        if (status >= 0) {
            ...
            if (procId == MultiProc_INVALIDID) {
                status = -1;
                OSA_printf("Invalid <Core name> specified!\n");
                FirmwareLoad_printUsageInfo ();
            }
        }
    }
...
Void FirmwareLoad_printUsageInfo (Void)
{
    ...
    OSA_printf ("Note: [File path] argument is only required when first"
                 " argument is 'startup' \n");
}
