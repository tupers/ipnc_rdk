...
Int32 MultiCh_GetprfLoad(SystemCommon_DanimicParams *pPrm)
{
    UInt32 procId;

    procId = SYSTEM_LINK_ID_DSP;

	if(procId == SYSTEM_LINK_ID_INVALID)
	{
		return -1;
	}

	System_linkControl(procId,SYSTEM_COMMON_CMD_CPU_LOAD_CALC_GET,pPrm,sizeof(SystemCommon_DanimicParams),TRUE);
