...
#include <mcfw/interfaces/ti_mjpeg.h>
...
MultiCh_ipcBitsInitCreateParams_SimcopBitsInHLOS(IpcBitsInLinkHLOS_CreateParams
                                                 * cp)
{
    MJPEG_CALLBACK_S *callback;

    Ptr cbCtx;

    cp->baseCreateParams.noNotifyMode = MULTICH_IPCBITS_NO_NOTIFY_BITSINHLOS;
    Mjpeg_getCallbackInfo(&callback, &cbCtx);
    cp->cbFxn = callback->newDataAvailableCb;
    cp->cbCtx = cbCtx;
    cp->baseCreateParams.notifyNextLink = FALSE;
    /* Previous link of bitsInHLOS is bitsOutRTOS. So, notifyPrevLink should
     * be set to false if bitsInHLOS is to operate in NO_NOTIFY_MODE */
    cp->baseCreateParams.notifyPrevLink =
        !(MULTICH_IPCBITS_NO_NOTIFY_BITSINHLOS);
}
