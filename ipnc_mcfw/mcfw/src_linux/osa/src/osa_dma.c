...
int OSA_dmaCopy(unsigned char *srcPhysAddr, Uint8 *dstPhysAddr, int width, int height,unsigned srcOffset,unsigned dstOffset)
{
  int status;
  ...
  unsigned start = OSA_getCurTimeInMsec();
  unsigned end = 0;
  ...
  if(status==OSA_SOK) {
    ...
    copy2D.srcOffsetH = srcOffset;
    copy2D.dstOffsetH = dstOffset;
